function [features, feature_names, comp_times, comp_times_names] = ...
    univariate_linear_features(input_signal, fs, psd_method, plotFigure)


% Input:
% - input_signal: signal to analyse
% - fs: sampling frequency
% - psd_method: method to compute the power spectral density
...
if strcmp(psd_method, 'welch')
    ...
    % FFT squared modulus
    v_PowSpec = fft(input_signal);
    v_PowSpec = v_PowSpec.* conj(v_PowSpec);
    v_PowSpec = v_PowSpec./ fs;
    v_Freq = (0:length(v_PowSpec) - 1).* (fs / length(v_PowSpec));
    s_LenHalf = floor(length(v_Freq)./ 2);
    psd_signal = v_PowSpec(1:s_LenHalf);
    frequency_vector = v_Freq(1:s_LenHalf);
    % s_TotalPow = sum(psd_signal);
end
...
%% Decorrelation time (BouAssi2017, Valderrama2012)
...
if isempty(decorrelation_time)
    decorrelation_time=NaN;
end
ct_dec_time = toc;
...
if plotFigure
    ...
    legend_entries = [legend_entries, {'SEF50'}];
